...
const folderName = `Nuutapao-Tools-v${version}`;
const targetDir = path.join(githubSourceDir, folderName);
const zipFile = path.join(githubSourceDir, `Nuutapao-Tools-v${version}-Source.zip`);

console.log(`📦 Packaging clean source code for GitHub release (${folderName})...`);
...
function copyRecursive(src, dest) {
  ...
  if (stat.isDirectory()) {
    ...
    for (const entry of entries) {
      if (['node_modules', 'dist', '.git', '.cache', '.tempmediaStorage', '.user_uploaded'].includes(entry)) continue;
...
try {
  execSync(`"${tarBin}" -a -c -f "${zipFile}" -C "${githubSourceDir}" "${folderName}"`, {
    cwd: rootDir,
    stdio: 'inherit'
  });
  ...
  execSync(`"${tarBin}" -cf "${zipFile}" -C "${githubSourceDir}" "${folderName}"`, {
    cwd: rootDir,
    stdio: 'inherit'
  });
}
