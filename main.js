...
// Prevent duplicate windows when "Run in Background" is on and user re-launches
...
function createTray() {
  ...
  let trayImage;
  // On Windows, ICO files provide crisp multi-resolution rendering across all DPI scaling levels
  if (process.platform === 'win32' && fs.existsSync(icoPath)) {
    ...
    trayImage = nativeImage.createFromPath(rootIco);
  } else if (fs.existsSync(logoPng)) {
    const raw = nativeImage.createFromPath(logoPng);
    trayImage = raw.isEmpty() ? raw : raw.resize({ width: 16, height: 16, quality: 'best' });
  } else {
