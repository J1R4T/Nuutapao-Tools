...
// Skip unnecessary GPU info collection that delays startup
app.commandLine.appendSwitch('disable-gpu-driver-bug-workarounds');
app.commandLine.appendSwitch('enable-zero-copy');
// Use faster compositing for transparent windows
app.commandLine.appendSwitch('enable-features', 'CanvasOopRasterization');
// Skip costly Windows occlusion calculation that delays window show
...
function createWindow() {
  ...
  const winIcon = fs.existsSync(icoPath) ? icoPath : (fs.existsSync(pngPath) ? pngPath : logoPath);
  ...
  // Use 'did-finish-load' instead of 'ready-to-show' — it fires earlier
  // because it doesn't wait for the full transparent compositing pass
...
// Check if an existing installation is detected at the target path
ipcMain.handle('check-existing', (event, targetPath) => {
  if (!targetPath) return false;
  // Look for the exe in the given path
  return (
    fs.existsSync(path.join(targetPath, 'Nuutapao Tools.exe')) ||
    fs.existsSync(path.join(targetPath, 'Nuutapao Downloader.exe'))
  );
});
...
function getWindowsSpecialFolders() {
  ...
  return { desktops: Array.from(desktops).filter(d => fs.existsSync(d) || d.includes('Desktop')), programs };
}
...
:: Remove user data and settings for a clean uninstall
echo Removing user data and settings...
if exist "%APPDATA%\\nuutapao-tools" rmdir /s /q "%APPDATA%\\nuutapao-tools" 2>nul
if exist "%APPDATA%\\Nuutapao Tools" rmdir /s /q "%APPDATA%\\Nuutapao Tools" 2>nul
if exist "%APPDATA%\\Nuutapao Downloader" rmdir /s /q "%APPDATA%\\Nuutapao Downloader" 2>nul
if exist "%USERPROFILE%\\.nuutapao-downloader" rmdir /s /q "%USERPROFILE%\\.nuutapao-downloader" 2>nul
...
function registerUninstall(targetPath, uninstallBatPath) {
  try {
    ...
    const cmds = [
      `reg add "${regKey}" /v "DisplayName" /t REG_SZ /d "Nuutapao Tools V.4.0.0" /f`,
      ...
      `reg add "${regKey}" /v "DisplayVersion" /t REG_SZ /d "4.0.0" /f`,
...
// Clean all user data directories for a fresh start
function cleanUserData() {
  const appData = process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming');
  const userProfile = process.env.USERPROFILE || os.homedir();
  const dataDirs = [
    path.join(appData, 'nuutapao-tools'),
    path.join(appData, 'Nuutapao Tools'),
    path.join(appData, 'Nuutapao Downloader'),
    path.join(userProfile, '.nuutapao-downloader')
  ];
  const cleaned = [];
...
ipcMain.handle('start-installation', async (event, { targetPath, createDesktopShortcut, createStartMenuShortcut }) => {
  try {
    ...
    try {
      execSync('taskkill /F /IM "Nuutapao Tools.exe" 2>nul', { stdio: 'ignore' });
      execSync('taskkill /F /IM "Nuutapao Downloader.exe" 2>nul', { stdio: 'ignore' });
    } catch (_) {}

    // Clean old installation if exists (reinstall scenario)
    if (fs.existsSync(targetPath)) {
      mainWindow.webContents.send('install-log', 'Removing previous installation...');
      mainWindow.webContents.send('install-progress', 2);
      try {
        fs.rmSync(targetPath, { recursive: true, force: true });
      } catch (_) {}
    }

    // Clean all user data for a fresh start
    mainWindow.webContents.send('install-log', 'Cleaning old user data for fresh start...');
    mainWindow.webContents.send('install-progress', 3);
    const cleaned = cleanUserData();
    for (const dir of cleaned) {
      mainWindow.webContents.send('install-log', `Removed: ${dir}`);
    }
    ...
    if (fs.existsSync(payloadZip)) {
      ...
      const tarProcess = spawn(tarBin, ['-xvf', payloadZip, '-C', targetPath]);
      let fileCount = 0;
    ...
    if (createDesktopShortcut) {
      for (const desktopDir of specialFolders.desktops) {
        ...
        const ok = createWindowsShortcut(exePath, desktopShortcut, 'Nuutapao Tools V.4.0.0');
        if (ok) {
    ...
    if (createStartMenuShortcut) {
      ...
      const ok = createWindowsShortcut(exePath, startMenuShortcut, 'Nuutapao Tools V.4.0.0');
      if (ok) {
