...
describe('media runtime API with deterministic adapters', () => {
  ...
  test('starts a clipper download with timeSection and passes --download-sections and keyframe cuts', async () => {
    let capturedArgs = [];
    server.setRuntimeAdapters({
      getToolPath: () => executable,
      exec: (command, callback) => callback?.(),
      spawn: (command, args) => {
        if (args.includes('--newline')) {
          capturedArgs = args;
        }
        return createMediaSpawn()(command, args);
      }
    });
    const outputPath = path.join(root, 'clipper-downloads');
    const response = await request('/api/download', {
      ...
      body: JSON.stringify({
        ...
        outputPath,
        timeSection: { start: '00:01:00', end: '00:02:30' }
      })
    });
    assert.equal(response.status, 200);
    await new Promise(resolve => setTimeout(resolve, 30));

    assert.ok(capturedArgs.includes('--download-sections'), 'args should include --download-sections');
    const sectionIndex = capturedArgs.indexOf('--download-sections');
    assert.equal(capturedArgs[sectionIndex + 1], '*00:01:00-00:02:30');
    assert.ok(capturedArgs.includes('--force-keyframes-at-cuts'), 'args should include --force-keyframes-at-cuts');
    assert.ok(capturedArgs.some(arg => arg.includes('[00.01.00-00.02.30]')), 'output template should contain formatted time range');

    const downloads = await (await request('/api/downloads')).json();
    assert.equal(downloads.history.at(0).timeSection, '00:01:00-00:02:30');
    server.setRuntimeAdapters({ spawn: createMediaSpawn(), getToolPath: () => executable, exec: (command, callback) => callback?.() });
  });

  test('starts a clipper download with section string and passes --download-sections', async () => {
    let capturedArgs = [];
    server.setRuntimeAdapters({
      getToolPath: () => executable,
      exec: (command, callback) => callback?.(),
    ...
    const outputPath = path.join(root, 'clipper-downloads-str');
    const response = await request('/api/download', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        url: 'https://example.test/video',
        outputPath,
        section: '00:00:15-00:00:45'
      })
    });
    assert.equal(response.status, 200);
    await new Promise(resolve => setTimeout(resolve, 30));

    assert.ok(capturedArgs.includes('--download-sections'));
    const sectionIndex = capturedArgs.indexOf('--download-sections');
    assert.equal(capturedArgs[sectionIndex + 1], '*00:00:15-00:00:45');
    server.setRuntimeAdapters({ spawn: createMediaSpawn(), getToolPath: () => executable, exec: (command, callback) => callback?.() });
  });
