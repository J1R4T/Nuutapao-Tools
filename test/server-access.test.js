...
describe('yt-dlp Error Classification', () => {
  ...
  test('classifies TikTok rehydration and blocked IP errors', () => {
    const rehydrationMsg = 'ERROR: [TikTok] 7106594312292453678: Unable to extract universal data for rehydration; please report this issue';
    const rehydrationClassified = classifyYtDlpError(rehydrationMsg);
    assert.ok(rehydrationClassified.includes('TikTok') && rehydrationClassified.includes('unexpected response'));

    const blockedMsg = 'ERROR: [TikTok] 7106594312292453678: Your IP address is blocked from accessing this post';
    const blockedClassified = classifyYtDlpError(blockedMsg);
    assert.ok(blockedClassified.includes('TikTok') && blockedClassified.includes('unexpected response'));
  });
...
describe('URL Cleaning & Cookie Auto-Conversion', () => {
  test('isTikTokUrl correctly detects all TikTok URL variants', () => {
    assert.equal(isTikTokUrl('https://www.tiktok.com/@username/video/7222341739086892293'), true);
    assert.equal(isTikTokUrl('https://vt.tiktok.com/ZSjabc/'), true);
    assert.equal(isTikTokUrl('https://vm.tiktok.com/ZM12345/'), true);
    assert.equal(isTikTokUrl('https://www.youtube.com/watch?v=12345'), false);
    assert.equal(isTikTokUrl('https://www.instagram.com/reel/C12345/'), false);
...
describe('UTF-8 & Multilingual Encoding Support', () => {
  ...
  test('buildPlatformArgs does not inject conflicting user-agent for TikTok', () => {
    const args = buildPlatformArgs('https://www.tiktok.com/@user/video/1234567890');
    assert.ok(!args.includes('--user-agent'));
    assert.ok(!args.includes('--referer'));
    assert.ok(args.includes('--encoding'));
  });

  test('buildPlatformArgs preserves Instagram headers without modification', () => {
    const args = buildPlatformArgs('https://www.instagram.com/p/C12345/');
    assert.ok(args.includes('--user-agent'));
    assert.ok(args.includes('--referer'));
  });
