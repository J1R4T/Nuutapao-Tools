...
// ── Assets Finder API ─────────────────────────────────────────────────────
function getAssetsApiKeys() {
  const st = getStateStore().get();
  return {
    pixabay: (st.settings?.pixabayApiKey || '').trim(),
    giphy: (st.settings?.giphyApiKey || '').trim()
  };
}
...
function httpsGet(url) {
  return new Promise((resolve, reject) => {
    ...
    mod.get(url, { headers: { 'User-Agent': 'NuutapaoTools/3.1' } }, (resp) => {
      let data = '';
      resp.on('data', chunk => { data += chunk; });
      resp.on('end', () => {
        try { resolve(JSON.parse(data)); }
        catch { reject(new Error('Invalid JSON from upstream API')); }
      });
    }).on('error', reject);
  });
}

function searchWebAssets(q, type = 'photo', page = 1, perPage = 30) {
  return new Promise((resolve, reject) => {
    const isGif = type === 'gif';
    const first = (page - 1) * perPage + 1;
    let targetUrl = `https://www.bing.com/images/search?q=${encodeURIComponent(q)}&form=HDRSC2&first=${first}`;
    if (isGif) {
      targetUrl += '&qft=+filterui:photo-animatedgif';
    } else {
      targetUrl += '&qft=+filterui:photo-photo';
    }
    ...
    const handleResp = (resp) => {
      if (resp.statusCode >= 300 && resp.statusCode < 400 && resp.headers.location) {
        ...
        return mod.get(nextUrl, options, handleResp).on('error', reject);
      }
      let body = '';
      ...
      resp.on('end', () => {
        try {
          ...
          const regex = /m="({.+?})"/g;
          let m;
          while ((m = regex.exec(body)) !== null && items.length < perPage) {
            try {
              const unescaped = m[1].replace(/&quot;/g, '"').replace(/&amp;/g, '&').replace(/&lt;/g, '<').replace(/&gt;/g, '>');
              const parsed = JSON.parse(unescaped);
              if (!parsed.murl) continue;

              let domain = 'Google / Web';
              try {
                if (parsed.purl) {
                  const u = new URL(parsed.purl);
                  domain = u.hostname.replace(/^www\./, '');
                }
              } catch {}

              const title = (parsed.t || parsed.desc || q)
                .replace(/[\uE000-\uF8FF]/g, '')
                .replace(/<[^>]+>/g, '')
                .trim();

              // Skip AI-generated content to ensure authentic photos and Gen Z aesthetics
              const lowerTitle = title.toLowerCase();
              const purlLower = (parsed.purl || '').toLowerCase();
              const murlLower = (parsed.murl || '').toLowerCase();
              const isAi = /\b(ai[\s_-]?art|ai[\s_-]?generated|midjourney|dall[\s_-]?e|stable[\s_-]?diffusion|craiyon|nightcafe|novelai|leonardo[\s_-]?ai|bing[\s_-]?image[\s_-]?creator)\b/i.test(lowerTitle) ||
                /(midjourney|civitai|leonardo\.ai|nightcafe\.studio|craiyon|artbreeder)/i.test(purlLower) ||
                /(midjourney|civitai|artbreeder)/i.test(murlLower);
              if (isAi) continue;

              // Filter out clipart, vector graphics, and svg stickers when user wants real photos
              if (!isGif) {
                const isVectorOrClipart = /\b(clipart|vector|svg|sticker|coloring page|png clipart)\b/i.test(lowerTitle) ||
                  /(vecteezy|vectorstock|freepik|freevector|cleanpng|pngtree)/i.test(purlLower) ||
                  murlLower.endsWith('.svg');
                if (isVectorOrClipart) continue;
              }

              const rawTags = (title + ' ' + q).toLowerCase().match(/\b[a-z0-9_-]{3,}\b/g) || [];
              const uniqueTags = [...new Set(rawTags)].slice(0, 6);
              const id = parsed.imageId || parsed.mid || (Math.random().toString(36).substring(2) + Date.now().toString(36));
              ...
              items.push({
                ...
                preview: parsed.murl,
                urls: {
                  small: parsed.turl || parsed.murl,
                  medium: parsed.murl,
                  large: parsed.murl,
                  original: parsed.murl
                },
                width: parseInt(parsed.width) || 0,
                height: parseInt(parsed.height) || 0,
                author: domain,
                authorUrl: parsed.purl || `https://www.google.com/search?q=${encodeURIComponent(q)}`,
                tags: uniqueTags,
                pageUrl: parsed.purl || ''
              });
            } catch {}
          ...
          resolve({
            total: items.length > 0 ? (page * perPage + 50) : 0,
            page,
            perPage,
            items
          });
    ...
    mod.get(targetUrl, options, handleResp).on('error', reject);
  });
}
...
app.get('/api/assets/search', async (req, res) => {
  ...
  const source = (req.query.source || 'google').toLowerCase();
  const type = req.query.type || 'photo';
  const page = Math.max(1, parseInt(req.query.page) || 1);
  const perPage = Math.min(50, Math.max(3, parseInt(req.query.per_page) || 30));

  const keys = getAssetsApiKeys();

  // Backward compatibility check for Pixabay and Giphy API keys
  if (source === 'giphy' && !keys.giphy) {
    return res.json({ setup_required: 'giphy', items: [], total: 0, page: 1, perPage });
  }
  if (source === 'pixabay' && !keys.pixabay) {
    return res.json({ setup_required: 'pixabay', items: [], total: 0, page: 1, perPage });
  }

  try {
    // 1. Google / Web (API-less, no key needed!)
    if (source === 'google' || source === 'web') {
      const data = await searchWebAssets(q, type, page, perPage);
      return res.json(data);
    }

    // 2. Giphy
    if (type === 'gif' || source === 'giphy') {
      if (!keys.giphy) {
        return res.json({ setup_required: 'giphy', items: [], total: 0, page: 1, perPage });
      }
      const offset = (page - 1) * perPage;
      const giphyUrl = `https://api.giphy.com/v1/gifs/search?api_key=${keys.giphy}&q=${encodeURIComponent(q)}&limit=${perPage}&offset=${offset}&rating=g&lang=en`;
      const gData = await httpsGet(giphyUrl);
      const items = (gData.data || []).map(g => ({
        id: g.id,
        source: 'giphy',
        type: 'gif',
        thumbnail: g.images?.fixed_width?.url || g.images?.preview_gif?.url || '',
        preview: g.images?.downsized_medium?.url || g.images?.fixed_width?.url || '',
        urls: {
          small: g.images?.fixed_width?.url || '',
          medium: g.images?.downsized_medium?.url || g.images?.downsized?.url || '',
          large: g.images?.downsized_large?.url || g.images?.original?.url || '',
          original: g.images?.original?.url || ''
        },
        width: parseInt(g.images?.original?.width) || 0,
        height: parseInt(g.images?.original?.height) || 0,
        author: g.user?.display_name || g.username || 'Giphy',
        authorUrl: g.user?.profile_url || `https://giphy.com/${g.username || ''}`,
        tags: g.title ? g.title.split(' ').filter(t => t.length > 1).slice(0, 5) : [],
        pageUrl: g.url || ''
      }));
      return res.json({
        total: gData.pagination?.total_count || items.length,
        page,
        perPage,
        items
      });
    ...
    // 3. Pixabay
    const pixabayType = type === 'vector' ? 'vector' : (type === 'illustration' ? 'illustration' : 'photo');
    const pixabayUrl = `https://pixabay.com/api/?key=${keys.pixabay}&q=${encodeURIComponent(q)}&image_type=${pixabayType}&page=${page}&per_page=${perPage}&safesearch=true`;
    const pData = await httpsGet(pixabayUrl);
    const items = (pData.hits || []).map(h => ({
      id: String(h.id),
      source: 'pixabay',
      type: pixabayType,
      ...
      urls: {
        ...
        original: h.fullHDURL || h.imageURL || h.largeImageURL || ''
      },
      width: h.imageWidth || 0,
      height: h.imageHeight || 0,
      author: h.user || 'Pixabay',
      authorUrl: `https://pixabay.com/users/${h.user_id}/`,
      tags: h.tags ? h.tags.split(',').map(t => t.trim()).filter(Boolean) : [],
      pageUrl: h.pageURL || ''
    }));
    res.json({
      total: pData.totalHits || items.length,
      page,
      perPage,
      items
    });
  } catch (err) {
    res.status(502).json({ error: `Asset search failed: ${err.message}` });
  }
});
...
app.post('/api/assets/download', async (req, res) => {
  ...
  const targetDir = savePath || path.join(os.homedir(), 'Downloads');
  ...
  try {
    await new Promise((resolve, reject) => {
      ...
      const doRequest = (requestUrl, redirectCount = 0) => {
        ...
        mod.get(requestUrl, { headers }, (resp) => {
          if (resp.statusCode >= 300 && resp.statusCode < 400 && resp.headers.location) {
            return doRequest(resp.headers.location, redirectCount + 1);
          }
...
// Browse folder dialog for Assets Finder save path
app.post('/api/assets/browse-folder', async (req, res) => {
  if (!electronDialog?.showOpenDialog) {
    return res.status(501).json({ error: 'Folder browsing is only available in the desktop app.' });
  }
  try {
    const { canceled, filePaths } = await electronDialog.showOpenDialog({
      title: 'Select folder to save assets',
      properties: ['openDirectory', 'createDirectory']
    });
    if (canceled || !filePaths?.length) return res.json({ cancelled: true });
    res.json({ cancelled: false, path: filePaths[0] });
  } catch (err) {
...
const tiktokResolvedCache = new Map();

function isTikTokUrl(url = '') {
  return /tiktok\.com|vm\.tiktok|vt\.tiktok/i.test(url || '');
}

async function resolveTikTokVideo(rawUrl) {
  let url = (rawUrl || '').trim();
  const cached = tiktokResolvedCache.get(url);
  if (cached && cached.expiresAt > Date.now()) {
    return cached.data;
  }

  // Expand short URLs (vt.tiktok.com, vm.tiktok.com, /t/)
  if (/vt\.tiktok|vm\.tiktok|\/t\//i.test(url)) {
    try {
      ...
      if (headRes.url && headRes.url !== url) {
        url = headRes.url.split('?')[0];
      }
    } catch {}
  }

  const ssstikUrl = 'https://ssstik.io';
  const getRes = await fetch(ssstikUrl, {
    headers: {
      'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36',
  ...
  const tt = match ? match[1] : null;
  if (!tt) throw new Error('Could not establish TikTok resolver session');
  ...
  if (!resHtml || resHtml.includes('Video currently unavailable') || resHtml.includes('panel critical')) {
    throw new Error('TikTok video is unavailable or private');
  }

  const title = resHtml.match(/<p class="maintext"[^>]*>([\s\S]*?)<\/p>/)?.[1]?.trim() || 'TikTok Video';
  const author = resHtml.match(/<h2>([\s\S]*?)<\/h2>/)?.[1]?.trim() || 'TikTok Creator';
  const cover = resHtml.match(/background-image:\s*url\(([^)]+)\)/)?.[1] ||
                resHtml.match(/<img class="result_author"[^>]*src="([^"]+)"/)?.[1] || '';

  const videoMatch = resHtml.match(/href="([^"]+)"[^>]*class="[^"]*without_watermark/i) ||
                     resHtml.match(/href="([^"]+)"[^>]*>Without watermark/i) ||
                     resHtml.match(/class="[^"]*download_link[^"]*"[^>]*href="([^"]+)"/i);
  const videoUrl = videoMatch ? videoMatch[1] : null;

  const audioMatch = resHtml.match(/href="([^"]+)"[^>]*class="[^"]*music/i) ||
                     resHtml.match(/href="([^"]+)"[^>]*>Download MP3/i);
  const audioUrl = audioMatch ? audioMatch[1] : null;

  if (!videoUrl && !audioUrl) {
    throw new Error('Unable to extract video download URL');
  }
  ...
  if (videoUrl) {
    formats.push({
      ...
      acodec: 'aac',
      fps: 30,
      note: 'Direct HD Stream (No Watermark)'
    });
  }
  if (audioUrl) {
    formats.push({
  ...
  tiktokResolvedCache.set(rawUrl, { data, expiresAt: Date.now() + 15 * 60 * 1000 });
  if (url !== rawUrl) {
    tiktokResolvedCache.set(url, { data, expiresAt: Date.now() + 15 * 60 * 1000 });
  }
...
function classifyYtDlpError(message = '', cookieSource = 'none') {
  ...
  if (/unexpected response|tiktok.*error|anti.?bot|challenge|blocked from accessing this post|universal data for rehydration/i.test(normalized) && /tiktok/i.test(normalized)) {
...
app.post('/api/info', async (req, res) => {
  ...
  // Fallback for TikTok if yt-dlp was challenged or blocked by anti-bot
  if (!result.ok && isTikTokUrl(url)) {
    try {
...
app.post('/api/download', (req, res) => {
  const { format, audioOnly, outputPath, subtitles, ext, embedThumbnail, timeSection, section } = req.body;
  const rawUrl = req.body.url;
  const url = extractCleanUrl(rawUrl);
  if (!url) return res.status(400).json({ error: 'URL is required' });
  const accessError = validateAccessSettings(req.body);
  ...
  const normalizedSection = formatTimeSection(timeSection || section);
  ...
  function normalizeFormatRequest(format) {
    ...
    if (numberMatch && ['4320', '2160', '1440', '1080', '720', '480', '360', '240'].includes(numberMatch[1])) {
      return { isBest: false, height: numberMatch[1] };
    }
    return { isBest: true, height: null };
  }

  function buildDownloadArgs(excludeCookies = false, forceLiveFallback = false, directStreamInfo = null) {
    const {
      url,
      format,
      audioOnly,
    ...
    const isDirectTikTok = Boolean(directStreamInfo && isTikTokUrl(url));
    const targetUrl = isDirectTikTok
      ? (audioOnly ? (directStreamInfo.directAudioUrl || directStreamInfo.directVideoUrl) : directStreamInfo.directVideoUrl)
      : url;
    ...
    if (!isDirectTikTok) {
      args.push('--print', 'before_dl:NuuMeta:%(title)s|||%(thumbnail)s');
    } else {
      args.push('--user-agent', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.0.0 Safari/537.36');
    }
    ...
    if (audioOnly) {
      ...
      if (audioExt === 'mp3') {
        args.push('--postprocessor-args', 'ffmpeg:-codec:a libmp3lame -b:a 320k -ar 48000');
      } else if (audioExt === 'm4a' || audioExt === 'aac') {
        args.push('--postprocessor-args', 'ffmpeg:-codec:a aac -b:a 256k -ar 48000');
      } else if (audioExt === 'wav') {
        args.push('--postprocessor-args', 'ffmpeg:-codec:a pcm_s16le -ar 48000');
      }
    } else {
      ...
      if (isDirectTikTok) {
        ...
        args.push('--postprocessor-args', 'ffmpeg:-c:v copy -c:a aac -b:a 192k -ar 48000 -movflags +faststart');
      } else if (isLive) {
        ...
        args.push('--postprocessor-args', 'ffmpeg:-c:a aac -b:a 192k -ar 48000');
        ...
        args.push('--postprocessor-args', 'ffmpeg:-c:a aac -b:a 192k -ar 48000 -movflags +faststart');
      } else {
        if (!parsedFormat.isBest && parsedFormat.height) {
          const h = parsedFormat.height;
          if (ytUrl) {
        ...
        if (videoExt === 'mov') {
          args.push('--postprocessor-args', 'ffmpeg:-c:a aac -b:a 192k -ar 48000');
        } else if (videoExt === 'webm' || videoExt === 'mkv') {
    ...
    if (isDirectTikTok && directStreamInfo?.title) {
      const safeTitle = directStreamInfo.title.replace(/[\\/:*?"<>|]/g, '').trim().substring(0, 150) || 'TikTok Video';
      const targetExt = audioOnly ? (ext || 'mp3') : (ext || 'mp4');
      if (normalizedSection) {
        const displayRange = normalizedSection.replace(/^\*/, '').replace(/[:]/g, '.');
        args.push('-o', path.join(downloadDir, `${safeTitle} [${displayRange}].${targetExt}`), targetUrl);
      } else {
        args.push('-o', path.join(downloadDir, `${safeTitle}.${targetExt}`), targetUrl);
      }
    } else if (normalizedSection) {
      const displayRange = normalizedSection.replace(/^\*/, '').replace(/[:]/g, '.');
      args.push('-o', path.join(downloadDir, `%(title)s [${displayRange}].%(ext)s`), targetUrl);
    } else {
      args.push('-o', path.join(downloadDir, '%(title)s.%(ext)s'), targetUrl);
    }
    return args;
  }
  ...
  function handleDownloadLine(rawLine) {
    ...
    if (line.includes('NuuMeta:')) {
      ...
      if (metaMatch) {
        ...
        if (extractedTitle) {
          download.title = normalizedSection
            ? `[${normalizedSection.replace(/^\*/, '')}] ${extractedTitle}`
            : extractedTitle;
        }
  ...
  let attemptedTikTokFallback = false;

  async function startProcess(excludeCookies = false, forceLiveFallback = false, forceFormatFallback = false, directStreamInfo = null) {
    if (directStreamInfo) {
      if (directStreamInfo.title) {
        download.title = normalizedSection
          ? `[${normalizedSection.replace(/^\*/, '')}] ${directStreamInfo.title}`
          : directStreamInfo.title;
      }
      if (directStreamInfo.thumbnail) download.thumbnail = directStreamInfo.thumbnail;
    } else if (isTikTokUrl(url)) {
      const cached = tiktokResolvedCache.get(url);
      if (cached && cached.expiresAt > Date.now() && cached.data) {
        directStreamInfo = cached.data;
        if (directStreamInfo.title) {
          download.title = normalizedSection
            ? `[${normalizedSection.replace(/^\*/, '')}] ${directStreamInfo.title}`
            : directStreamInfo.title;
    ...
    let args = buildDownloadArgs(excludeCookies, forceLiveFallback, directStreamInfo);
    ...
    proc.on('close', async (code) => {
      ...
      // If TikTok download failed with anti-bot or challenge error, resolve and retry via direct stream
      if (code !== 0 && !attemptedTikTokFallback && isTikTokUrl(url) && !directStreamInfo) {
        attemptedTikTokFallback = true;
        try {
          const resolved = await resolveTikTokVideo(url);
          if (resolved && (resolved.directVideoUrl || resolved.directAudioUrl)) {
            return startProcess(true, false, false, resolved);
          }
        } catch (e) {}
      }
      ...
      if (code === 0) {
        ...
        if (!download.filename && download.title && fs.existsSync(downloadDir)) {
          try {
            ...
            const cleanTitle = download.title.replace(/^\[[^\]]+\]\s*/, '').replace(/[\\/:*?"<>|]/g, '').trim().toLowerCase();
  ...
  if (isVideo) {
    ...
    if (outputIsAudio) {
      ...
      if (outputFormat === 'mp3') {
        ...
        args.push('-c:a', 'libmp3lame', '-b:a', br, '-ar', '48000');
        ...
        args.push('-c:a', 'aac', '-b:a', br, '-ar', '48000');
      ...
      if (outputFormat === 'mp4') {
        ...
        args.push('-c:v', 'libx264', '-preset', 'medium', '-crf', crf, '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-b:a', '192k', '-ar', '48000', '-movflags', '+faststart');
        ...
        const crf = qualityLevel === 'high' ? '18' : qualityLevel === 'medium' ? '23' : '28';
        args.push('-c:v', 'libx264', '-preset', 'medium', '-crf', crf, '-c:a', 'aac', '-b:a', '192k', '-ar', '48000');
      } else if (outputFormat === 'avi') {
        args.push('-c:v', 'libx264', '-c:a', 'mp3', '-b:a', '192k', '-ar', '48000');
      } else if (outputFormat === 'mov') {
        const crf = qualityLevel === 'high' ? '18' : qualityLevel === 'medium' ? '23' : '28';
        args.push('-c:v', 'libx264', '-preset', 'medium', '-crf', crf, '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-b:a', '192k', '-ar', '48000');
      } else {
        args.push('-c:v', 'libx264', '-c:a', 'aac', '-ar', '48000');
      }
    }
  } else if (isAudio) {
    if (outputFormat === 'mp3') {
