...
function getToolPath(name) {
  ...
  if (runtimeConfig.isPackaged && runtimeConfig.resourcesPath) {
    return path.join(runtimeConfig.resourcesPath, 'tools', `${name}${toolExt}`);
  }
  try {
...
function buildPlatformArgs(url = '', hasCookies = false, isLive = false) {
  ...
  if (/youtu\.?be|youtube\.com/i.test(url)) {
    ...
    } else if (/twitter\.com|x\.com|t\.co/i.test(url)) {
...
app.post('/api/download', (req, res) => {
  ...
  function buildDownloadArgs(excludeCookies = false, forceLiveFallback = false) {
    const hasCookies = Boolean(!excludeCookies && req.body.cookieSource && req.body.cookieSource !== 'none');
    const isLive = isLiveStream || forceLiveFallback;
    let args = [
      '--newline',
      '--no-simulate',
      '--progress',
    ...
    if (audioOnly) {
      ...
      args.push('-f', 'bestaudio/best', '-x', '--audio-format', audioExt, '--audio-quality', '0');
      ...
      if (isLive) {
        if (format && format !== 'best') {
          ...
          if (heightMatch) {
            ...
            args.push('-f', `best[height<=${h}]/bestvideo[height<=${h}]+bestaudio/best`);
          } else {
            args.push('-f', format);
          }
        } else {
  ...
  function startProcess(excludeCookies = false, forceLiveFallback = false) {
    const args = buildDownloadArgs(excludeCookies, forceLiveFallback);
